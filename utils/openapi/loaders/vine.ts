/**
 * references:
 * https://github.com/FriendsOfAdonis/FriendsOfAdonis/blob/main/packages/openapi/src/loaders/vine.ts
 */
...
export function parseCompilerNode(
	node: CompilerNode,
	refs: Refs,
...
export function parseObjectNode(
	node: ObjectNode,
	refs: Refs,
): OpenAPIV3.SchemaObject {
...
export function parseArrayNode(
	node: ArrayNode,
	refs: Refs,
): OpenAPIV3.SchemaObject {
	return {
...
export function parseLiteralNode(
	_node: LiteralNode,
	_refs: Refs,
): OpenAPIV3.SchemaObject {
	return {
...
export async function enrichType(
	...
	for (const message of error.messages) {
		...
		if (message.message === "TYPE") {
			set(schema.properties, `${fieldPath}.type`, message.rule)
		}
	}
}
...
export const VineTypeLoader: TypeLoaderFn = async (
	context,
	value,
	original,
) => {
	if (value instanceof VineValidator) {
		if (original === value) {
			context.logger.warn(
				`Vine Validators can only be loaded using Thunk '() => myValidator'`,
			)
			return
		}
